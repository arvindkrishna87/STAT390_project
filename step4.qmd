...
| Pros              | Cons              |
|------------------|-----------------|
| The patch will be generated from the points along the centerline, ensuring that the epithelium is well-positioned within the square patch.<br><br> Consistency across patches since the points are always centralized within the epithelial region. <br><br> | Code checking if the patch contains both stroma and epithelium is also time consuming, as it requires manual entry <br><br> If patch doesn’t contain both, you have to go back and try another one. |
