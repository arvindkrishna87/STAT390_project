...
**Aim**: Applies horizontal / vertical patching across the epithelium
for later use in model.
...
Directories: Defines input_folder (where processed images are stored)
and output_folder (where the results will be saved).

Directory structure:
`Step 4/ Processed_images_sub Filtered_images epithelium_patches_6_hori.py`

## Set Up

Place all desired sample images in “filtered_images” folder

Open and run `epithelium_patches_6_hori.py`. Dependencies include cv2,
numpy, os
